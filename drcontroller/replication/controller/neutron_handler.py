...
class NeutronApp(base_handler.BaseHandler):
    ...
    def delete_handle(message):
        ...
        ##
            ...
            endpoint, auth_token = self.keystone_handle(key_type='drc', service_type='subnet',endpoint_type='pyblicURL')
            drc_neutron = neutronclient.Client('2.0', endpoint_url=endpoint, token=auth_token)
    ...
    def put_handle(message):
        ...
        if network_type == 'networks':
            endpoint, auth_token = self.keystone_handle(key_type="drf", service_type='network', endpoint_type='publicURL')
            drf_neutron = neutronclient.Client('2.0', endpoint_url=endpoint, token=auth_token)
            url = message['Request']['url'].split('/')
        ...
        ##
            ...
            endpoint, auth_token = self.keystone_handle(key_type="drf", service_type='subnet', endpoint_type='publicURL')
            drf_neutron = neutronclient.Client('2.0', endpoint_url=endpoint, token=auth_token)
            url = message['Request']['url'].split('/')
            ...
            if status == 'active':
                pdb.set_trace()
                endpoint, auth_token = self.keystone_handle(key_type='drc', service_type='subnet', endpoint_type='publicURL')
                drc_neutron = neutronclient.Client('2.0', endpoint_url=endpoint, token=auth_token)
                ##
                ...
                subnet_params = {'id':drc_subnet_uuid,
                                 'allocation_pools':message['Response']['subnet']['allocation_pools'],
                                 'cidr':message['Response']['subnet']['cidr'],
                                 'dns_nameservers':message['Response']['subnet']['dns_nameservers'],
                                 'enable_dhcp':message['Response']['subnet']['enable_dhcp'],
                                 'gateway_ip':message['Response']['subnet']['gateway_ip'],
